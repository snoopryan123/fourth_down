...
generate_V_dataset <- function(kq, pq, pspread, momd, momds, mdmo, mdmos, confounders_dataset) {
  df = tibble()
  MAX_YTG = 15 #10
  for (ydstogo in 1:MAX_YTG) {
    dfy = tibble(
      ...
      market_OQOT_minus_DQDT = momd,
      market_OQOT_minus_DQDT_std = momds,
      market_OQDT_minus_DQOT = mdmo,
      market_OQDT_minus_DQOT_std = mdmos,
    )
    df = bind_rows(df, dfy)
  }
...
#   era_A = 3
# )
# ex2 = generate_V_dataset(kq=0, pq=0, pspread=0, ex2_confounders_dataset)
# ex2
## View(ex2)
...
P_FG <- function(fg_model_fit, dataset, og_method=FALSE) {
  ...
  dataset = dataset %>% select(yardline_100, kq)
  if (!og_method) {
...
E_punt <- function(punt_model_fit, dataset, og_method=FALSE) {
  ...
  dataset = dataset %>% select(yardline_100, pq)
  if (!og_method) {
...
P_convert <- function(go_model_fit, dataset, og_method=FALSE) {
  ...
  dataset = dataset %>% 
    select(ydstogo, yardline_100, posteam_spread, market_OQOT_minus_DQDT, market_OQOT_minus_DQDT_std) %>%
    mutate(down = 4) 
  if (!og_method) {
...
field_position_switcherooni <- function(dataset, next_ydl_opp, next_score_diff_opp) {
  ...
  dataset_1a = dataset %>% mutate(
    ...
    moomdd = market_OQOT_minus_DQDT,
    market_OQOT_minus_DQDT = market_OQDT_minus_DQOT,
    market_OQDT_minus_DQOT = moomdd,
    moomdd_std = market_OQOT_minus_DQDT_std,
    market_OQOT_minus_DQDT_std = market_OQDT_minus_DQOT_std,
    market_OQDT_minus_DQOT_std = moomdd_std,
    ### switch other stuff
...
get_coach_decision_freqs <- function(fourth_down_dataset, coach_model) {
  ...
  result = result %>% 
    rowwise() %>%
    mutate(
      decision_coach_usual = case_when(
        C_Go == max(C_Go, C_FG, C_Punt) ~ "Go",
        C_FG == max(C_Go, C_FG, C_Punt) ~ "FG",
        C_Punt == max(C_Go, C_FG, C_Punt) ~ "Punt",
      ),
      decision_coach_freq = case_when(
        C_Go == max(C_Go, C_FG, C_Punt) ~ C_Go,
        C_FG == max(C_Go, C_FG, C_Punt) ~ C_FG,
        C_Punt == max(C_Go, C_FG, C_Punt) ~ C_Punt,
      ),
    ) %>%
    ungroup()
  result
...
get_full_decision_making <- function(play_df, wp, og_method=FALSE, SE=FALSE, b_max=B, coachBaseline=FALSE, 
  ...
  momd = play_df$market_OQOT_minus_DQDT
  momds = play_df$market_OQOT_minus_DQDT_std
  mdmo = play_df$market_OQDT_minus_DQOT
  mdmos = play_df$market_OQDT_minus_DQOT_std
  ...
  dataset_V = generate_V_dataset(kq, pq, pspread, momd, momds, mdmo, mdmos, confounders_dataset)
  if (!post_TD) {
    ### FOURTH DOWN
    if (coachBaseline) {
...
plot_4thDownHeatmap <- function(decision_df, wp, og_method=FALSE, title=TRUE, legend_pos="right", 
  ...
  p = p +
    xlab("yards to opponent endzone") + ylab("yards to go") +
...
plot_Vs <- function(ddf, ytg, SE=FALSE, include_og=FALSE, wp=FALSE) {
  ...
  plot_decision = ddf_iytg %>%
    ...
    xlab("yards to opponent endzone") + ylab(ylab_) +
  ...
  if (boot_plot) {
    ...
    plot_decision = ddf_lookahead %>%
      ...
      scale_x_continuous(
        ...
        sec.axis = sec_axis(~.x+ydl-ytg_, breaks = seq(ydl-100,ydl+100,by=5), name="yards to opponent endzone")
      ) 
    plot_decision
    return(plot_decision)
  }
...
plot_gt_4th_down_summary <- function(play_df, ddf, decision_df=NULL, SE=FALSE, wp=TRUE, dec_conf_str=FALSE) {
  ...
  a2 = paste0(
    ...
    "Point Spread: ", play_df$posteam_spread, ", OQ-DQ: ", round(play_df$market_OQOT_minus_DQDT_std,2), 
    ", KQ: ", round(play_df$kq,2), ", PQ: ", round(play_df$pq,2)
  )
  a2
