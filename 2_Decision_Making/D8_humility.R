...
fname = paste0("results_humility_phi",phi,"/ddf_wp_se.csv")
if (file.exists(fname)) {
  ddf_wp_se = read_csv(fname)
} else {
  ddf_wp_se_lst = get_all_decision_making(df4d, wp=TRUE, SE=TRUE, coachBaseline=TRUE) ### takes ~5-15 minutes
  ddf_wp_se = ddf_wp_se_lst$Vs
  write_csv(ddf_wp_se, fname)
}

### plot
# bds_boot = c(0,  0.75, 0.9, 1)
# bds_boot = c(0,  0.70, 0.85, 1)
thirds = round(seq(0.5, 1, length.out = 4), 2)
bds_boot = thirds; bds_boot[1] = 0;
# quantile(ddf_wp_se$prop_decision, probs=c(1/3,2/3) )
bds_boot
...
plot_df_overconfidence =  plot1_df %>% distinct(wp_gain_bin, boot_perc_bin, p) 
plot_df_freq = plot1_df %>% group_by(wp_gain_bin) %>%
  summarise(count = n()) %>% mutate(perc_wpgb = count/sum(count))
# plot_df_humility = bind_rows(plot_df_overconfidence, plot_df_freq)
plot_df_humility = left_join(plot_df_overconfidence, plot_df_freq)
...
plot_humility =
  ...
  scale_fill_manual(name="", values = colors_boot) +
  # ylab("count") +
  ylab("frequency") +
  theme(
    axis.text = element_text(size=20),
    axis.text.x = element_text(angle = 40, vjust = 1, hjust=1, size=18),
...
# plot_humility
ggsave(paste0("results_humility_phi",phi,"/plot_overconfidence_1.png"), width=10, height=7)

##########################################
# plot_overconfidence =
#   plot_df_overconfidence %>%
#   ggplot(aes(x = wp_gain_bin)) +
...
# # plot_overconfidence
# ggsave("results_humility/plot_h_overconfidence.png",
...
#   theme(axis.text.x = element_text(angle = 40, vjust = 1, hjust=1, size=15))
# # plot_freq
# ggsave("results_humility/plot_h_freq.png",
...
gtsave(plays_summary_gt, paste0("results_humility_phi",phi,"/plays_summary_gt.png"))
...
xlab_ = paste0('proportion of "confident" fourth downs\n', "in which the coach's decision matches ours")
plot_coach_decs = 
  df_coach_decs %>%
...
ggsave(paste0("results_humility_phi",phi,"/plot_coach_decs.png"), 
       plot_coach_decs, width=12.5, height=16)
...
gtsave(gt(NflTooConservativeDf), paste0("results_humility_phi",phi,"/NflTooConservativeDf.png"))
