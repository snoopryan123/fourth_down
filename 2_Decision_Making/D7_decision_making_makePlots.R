...
plays =  data_full_0A %>% filter(row_idx %in% play_idxs) %>% mutate(preset_play=FALSE)
plays
dim(plays)

sum(is.na(data_full_0A$kq))
sum(is.na(data_full_0A$pq))

sum(is.na((data_full_0A %>% filter(season >= 2006 & season <= 2021))$pq))
...
# for (j in 6) {
  ...
  pq_i = play$pq
  qb_i = play$qb_name
  momds_i = round(play$market_OQOT_minus_DQDT_std,3)
  ...
  play_desc_i = paste0(play$down, "th down and ", play$ydstogo, " yards to go at the ", play$yardline_100, " yardline \n",
                       ...
                       " \n the spread is ", ps_i,
                       "\n the market-derived OQ minus DQ is ", momds_i,
  ...
  if (!preset_play_i) {
    textPlot(paste0("plots_decisions_phi",phi,"/","plot_decisions_play_", i, "desc"), play_str)
  }
  ...
  ggsave(paste0("plots_decisions_phi",phi,"/", plot_prefix, ".png"), heatmap_i, width=9, height=7)
  ...
  ggsave(paste0("plots_decisions_phi",phi,"/", plot_prefix_se, ".png"), heatmap_ise, width=9, height=7)
  ...
  gtsave(aaa,  paste0("plots_decisions_phi",phi,"/", plot_prefix_se, "summary", ".png"))
  
  ddf_ise_all = ddf_ise_lst$Vs_all
  plot_boot_dist = plot_4th_down_boot_dist(play, ddf_ise_all, wp=wp)
  ggsave(paste0("plots_decisions_phi",phi,"/", plot_prefix_se, "boot_dist", ".png"), plot_boot_dist, width=6, height=5)
}
